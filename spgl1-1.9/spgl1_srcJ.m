function [x, r, g, info] = spgl1_srcJ( A, b, bcv, tau, sigma, x, options)
...
if singleTau
    ...
    logH = ' %5s  %13s  %13s  %9s  %6s  %6s  %6s  %6s\n';
    printf(logH, 'Iter', 'Objective', 'Relative Gap', 'gNorm', 'stepG', 'nnzX', 'nnzG', 'CVObjective');
else
    logB = ' %5i  %13.7e  %13.7e  %9.2e  %9.3e  %6.1f  %6i  %6i %13.7e';
    logH = ' %5s  %13s  %13s  %9s  %9s  %6s  %6s  %6s  %13s  %13s\n';
    printf(logH, 'Iter', 'Objective', 'Relative Gap', 'Rel Error', ...
        'gNorm', 'stepG', 'nnzX', 'nnzG', 'CVObjective', 'tau');
end
...
while 1
    ...
    rNorm   = norm(r, 2);
    rcvNorm = norm(rcv, 2);
    cvStk   = circshift(cvStk, 1);
    cvStk(1)= rcvNorm;
    gap     = r'*(r-b) + tau*gNorm;
    rGap    = abs(gap) / max(1, f);
%     rGap    = abs(gap) / max(sigma^2 / 2, f);
    aError1 = rNorm - sigma;
    aError2 = f - sigma^2 / 2;
%     rError1 = abs(aError1) / max(1, rNorm);
%     rError2 = abs(aError2) / max(1, f);
    rError1 = abs(aError1) / max(sigma, rNorm);
    rError2 = abs(aError2) / max(sigma^2 / 2, f);
...
%         testRelChange2 = (abs(f - fOld) <= 1e-2 * f * (abs(rNorm - sigma))/max(sigma, rNorm));
...
%             petscfilename = ['X_' num2str(iter) '.J'];
%             PetscBinaryWrite(petscfilename, complex(x), 'complex', true);
        end
    end
    ...
    if logLevel >= 2 || singleTau || printTau || iter == 0 || stat
        ...
        if singleTau
            printf(logB, iter, rNorm, rGap, gNorm, log10(stepG), nnzX, nnzG, rcvNorm);
            if subspace
                printf('  %s', subFlag);
            end
        else
            printf(logB, iter, rNorm, rGap, rError1, gNorm, log10(stepG), nnzX, nnzG, rcvNorm);
    ...
    %==================================================================
    iter = iter + 1;
%     xStack(:, mod(iter, 3)+1) = x; 
    xOld = x;  fOld = f;  gOld = g;  rOld = r;
%     if (cvStk(1) > cvStk(2) && cvStk(2) > cvStk(3)) 
%         if saveiter - (iter-3) == -1 || cvStk(3) >= cvmin
%             saveiter = iter-3;
%         else
%             cvmin    = cvStk(3);
%             saveiter = iter-3;
%             petscfilename = ['X_' num2str(saveiter) '.J'];
%             PetscBinaryWrite(petscfilename, complex(xStack(:, mod(saveiter+1, 3)+1)), 'complex', true);
%         end
%     end
%     if iter == maxIts
%         petscfilename = ['X_' num2str(maxIts) '.J'];
%         PetscBinaryWrite(petscfilename, complex(xOld), 'complex', true);
%     end
    if cvStk(1)<=cvmin
        xOpt = x; cvmin = cvStk(1); iterOpt = iter;
    elseif iterOpt + 30 < iter;
        maxIts = iter; 
    end
    ...
    try
        ...
        %---------------------------------------------------------------
        [f, x, r, nLine, stepG, lnErr, rcv] = ...
            spgLineCurvy(x, gStep*g, max(lastFv), @Aprod, b, @project, tau, bcv);
        ...
        if lnErr
            ...
            gtd  = g'*dx;
            [f, x, r, nLine, lnErr, rcv] = spgLine(f, x, dx, gtd, max(lastFv), @Aprod, b, bcv);
            nLineTot = nLineTot + nLine;
        end
        if lnErr
...
function [fNew, xNew, rNew, iter, err, rcvNew] = spgLine(f, x, d, gtd, fMax, Aprod, b, bcv)
...
function [fNew, xNew, rNew, iter, step, err, rcvNew] = spgLineCurvy(x, g, fMax, Aprod, b, project, tau, bcv)
